...
static char *textDup(const char *s, const char *err)
{
    size_t n;
    char *p;
    if (s == NULL || s[0] == '\0')
...
static void init(int argc, char* argv[])
{
  ...
  // Set the shell
  shell = textDup(getenv("SHELL"), NULL);

  if (shell == NULL)
    {
      if (info == NULL || info->pw_shell == NULL || info->pw_shell[0] == '\0')
        {
          printf("%s\n", "Couldn't determine user shell");
          exit(1);
        }

      shell = textDup(info->pw_shell, "Couldn't initialize shell");
    }


  // Set the editor
  editor = textDup(getenv("EDITOR"), NULL);

  if (editor == NULL)
    editor = textDup("vi", "Couldn't initialize editor"); 
  ...
  if (xdg == NULL || xdg[0] == '\0')
    {
      cache_path = pathJoin(info->pw_dir,
                            "/.config/cfiles",
                            "Couldn't initialize cache path");
    }
  else
    {
      cache_path = pathJoin(xdg,
                            "/cfiles",
                            "Couldn't initialize cache path");
    }
  ...
  if (stat(cache_path, &st) == -1)
    {
      if (mkdir(cache_path, 0751) == -1)
        {
          printf("Couldn't create cache directory\n");
          exit(1);
        }
    }
  ...
  clipboard_path = pathJoin(cache_path, "/clipboard", "Couldn't initialize clipboard path");

  // Set the path for the bookmarks file
  bookmarks_path = pathJoin(cache_path, "/bookmarks", "Couldn't initialize bookmarks path");

  // Set the path for the scripts directory
  scripts_path = pathJoin(cache_path, "/scripts", "Couldn't initialize scripts path");

  // Set the path for the temp clipboard file
  temp_clipboard_path = pathJoin(cache_path, "/clipboard-tmp", "Couldn't initialize temp clipboard path");

  // Set the path for trash
  trash_path = pathJoin(info->pw_dir, "/.local/share/Trash/files", "Couldn't initialize trash path");
